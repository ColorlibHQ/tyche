...
# Tyche keeps its stylesheets in assets/css, one per colour scheme, so scan them all
# rather than just a single style.css at the theme root.
css_files = [theme_dir/'style.css', theme_dir/'rtl.css']
css_files += sorted((theme_dir/base/'css').glob('*.css'))
