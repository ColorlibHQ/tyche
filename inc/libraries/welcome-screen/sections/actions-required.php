...
<div class="feature-section action-required demo-import-boxed" id="plugin-filter">
	...
	if ( ! empty( $tyche_required_actions ) ):
		...
		foreach ( $tyche_required_actions as $tyche_required_action_key => $tyche_required_action_value ):
			$hidden = false;
			if ( @$tyche_show_required_actions[ $tyche_required_action_value['id'] ] === false ) {
				$hidden = true;
			}
            ...
            <div class="tyche-action-required-box">
				...
				<?php else: ?>
                    <span data-action="add" class="dashicons dashicons-hidden tyche-required-action-button"
                          id="<?php echo esc_attr( $tyche_required_action_value['id'] ); ?>"></span>
				<?php endif; ?>
                <h3><?php if ( ! empty( $tyche_required_action_value['title'] ) ): echo esc_html( $tyche_required_action_value['title'] ); endif; ?></h3>
                <p>
					<?php if ( ! empty( $tyche_required_action_value['description'] ) ): echo esc_html( $tyche_required_action_value['description'] ); endif; ?>
					<?php if ( ! empty( $tyche_required_action_value['help'] ) ): echo '<br/>' . wp_kses_post( $tyche_required_action_value['help'] ); endif; ?>
                </p>
				<?php
				if ( ! empty( $tyche_required_action_value['plugin_slug'] ) ) {
					...
					?>
                    <p class="plugin-card-<?php echo esc_attr( $tyche_required_action_value['plugin_slug'] ) ?> action_button <?php echo ( $active['needs'] !== 'install' && $active['status'] ) ? 'active' : '' ?>">
