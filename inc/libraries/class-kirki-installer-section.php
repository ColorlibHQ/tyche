		...
		protected function render() {
			...
			<div style="padding:10px 14px;">
				<?php if ( ! $installed ) : ?>
					...
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $plugin_install_url ); ?>" aria-label="Install Kirki Toolkit now" data-name="Kirki Toolkit"><?php esc_html_e( 'Install Now', 'tyche' ); ?></a>
				<?php else : ?>
					<?php esc_attr_e( 'Kirki Toolkit plugin is required to take advantage of this theme\'s features in the customizer.', 'tyche' ); ?>
					<a class="install-now button-secondary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="Activate Kirki Toolkit now" data-name="Kirki Toolkit"><?php esc_html_e( 'Activate Now', 'tyche' ); ?></a>
				<?php endif; ?>
			</div>
			<?php
		}
